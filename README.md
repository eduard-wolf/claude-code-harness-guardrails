...
- **[guards/check-counts.mjs](guards/check-counts.mjs)** — every count
  *marked* for the guard in this README (like the trap count above) is
  ...
  trap reference in this README — a "(Trap 11)<!-- trap-ref: 11 YAML -->"
  has to still point at the entry it names, and a reference left unmarked
  is red as well, so renumbering the catalog cannot leave the prose quietly
  wrong. It holds one unmarked invariant on top: as many `**Guard:**` lines
  in the catalog as entries, which is what makes "every entry here ends in
